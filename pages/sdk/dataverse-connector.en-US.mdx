...
const createCapability = async () => {
  const pkh = await dataverseConnector.createCapability({
    app,
...
The `pkh` is optional. If not specified, it will return all streams under the model.
...
const res = await dataverseConnector.updateStream({
  streamId,
  streamContent: {
    appVersion: '0.0.1',
    text: 'hello world!',
...
const res = await dataverseConnector.monetizeFile({
  streamId,
  datatokenVars: {
