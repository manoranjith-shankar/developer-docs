...
Creating a data stream under the corresponding model means creating structured data in the application. For example, creating a data stream using the [`createStream`](/sdk/apis/primitives/createStream) method under the `post` model means publishing a tweet on a social network.
